...
public class LetterStringFinder {
  ...
  public LetterStringFinder(String str){
    //
    mainStr = str;
    mainStr += '\n';
    //
  ...
  private void initPeople(){
    ...
    //
    name = "Jimson Mathew";posts.add("HoD,CSE");
  ...
  public Vector<String> getNamesList(){
    ...
    for (Person p : people){
      ...
      while(t<copiedStr.length()){
        ...
        if(nm && tmp.contains(x.toLowerCase())){
          ls.add(x);break;
        }
        boolean ps = false;
        ArrayList<String> posts = p.getPosts();
        for(String pst : posts){
          if(tmp.contains(pst.toLowerCase())){
            if(!nm)ls.add(pst);else ls.add(x);
            ps=true;break;
          }
        }
        if(ps)break;
